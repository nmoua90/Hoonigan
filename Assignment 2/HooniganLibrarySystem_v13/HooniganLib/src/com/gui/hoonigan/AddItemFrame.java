	...
	public void addItem(){
		...
		if(selectedChoice.compareToIgnoreCase("BOOK") == 0){
			if(name.isEmpty() || id.isEmpty() || artist.isEmpty() || libIDfield.getText().isEmpty())
				...
				globalVariables.addToItemsHashMap(new BookItem(name, selectedChoice, id, artist, libraryCode));
		}else if(selectedChoice.compareToIgnoreCase("CD") == 0){
			if(name.isEmpty() || id.isEmpty() || artist.isEmpty() || libIDfield.getText().isEmpty())
				missingFieldsMessage();
			else
				globalVariables.addToItemsHashMap(new CD(name, selectedChoice, id, artist, libraryCode));
		}else if(selectedChoice.compareToIgnoreCase("MAGAZINE") == 0){
			if(name.isEmpty() || id.isEmpty() || libIDfield.getText().isEmpty())
				missingFieldsMessage();
			else
				globalVariables.addToItemsHashMap(new Magazine(name, selectedChoice, id, volume, libraryCode));
		}else{
			if(name.isEmpty() || id.isEmpty() || libIDfield.getText().isEmpty())
				missingFieldsMessage();
			else
				globalVariables.addToItemsHashMap(new DVD(name, selectedChoice, id, libraryCode));
		}
	}
