 ...
 * 
 * Outdated Class, replaced by HashMapItems...
 * 
...
public class InitializeProgram {
	...
	public String getAllLibraryElements(){
		...
		while(itr.hasNext()){
			Item li = libraryMap.get(itr.next());
			line += li.toString() + "\n";
		}
	 ...
	 */
	public Item searchQueryResult(int checkStatus){
		Item found = null;
	 ...
	 */
	public void doLibraryOperations(int checkStatus, Item found){
		try{
	...
	public void runProgram(){
		...
		while(runProgram == 1){
			...
			Item queryResult = searchQueryResult(choice);
